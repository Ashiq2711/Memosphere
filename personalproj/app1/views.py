...
from app1.models import logintable,usertable,personaltable,educationtable,medicaltable,accounttable,employeetable
...
def employee(request):
    if request.method == "POST":
        ...
        i = employeetable(Name=a, job_title=b, work_email =c, work_phone=d, date_of_joining=e, employment_type=f, employee_id=g, office_location=h)
        i.save()
        return redirect('submit')
...
def employeedata(request):
    x = usertable.objects.get(id=request.session["member_id"])
    print(x.Name)
    c = employeetable.objects.filter(Name=x.Name)

    if request.method == "POST":
        c.Name = request.POST["name1"]
        c.job_title = request.POST["name2"]
        c.work_email = request.POST["name3"]
        c.work_phone = request.POST["name4"]
        c.date_of_joining = request.POST["name5"]
        c.employment_type= request.POST["name6"]
        c.employee_id = request.POST["name7"]
        c.office_location = request.POST["name8"]
        c.save()

    return render(request, "employeedata.html", {"employeekey": c})


def employeedit(request, id33):
    # Retrieve the object safely
    c = get_object_or_404(employeetable, id=id33)

    if request.method == "POST":
        c.Name = request.POST.get("name1", c.Name)
        c.job_title = request.POST.get("name2", c.job_title)
        c.work_email = request.POST.get("name3", c.work_email)
        c.work_phone = request.POST.get("name4", c.work_phone)

        # Handle date input safely
        date_of_joining = request.POST.get("name5", None)
        if date_of_joining:
            try:
                c.date_of_joining = date_of_joining
            except ValidationError:
                return HttpResponse("Invalid date format. Use YYYY-MM-DD.")

        c.employment_type = request.POST.get("name6", c.employment_type)
        c.employee_id = request.POST.get("name7", c.employee_id)
        c.office_location = request.POST.get("name8", c.office_location)

        # Save changes to the database
        c.save()
        return redirect('update')

    # Render the edit form with current object data
    return render(request, "employeedit.html", {'key33': c})

def emdelete(request,id35):
    c=employeetable.objects.get(id=id35)
    c.delete()
    return redirect('deleted')
